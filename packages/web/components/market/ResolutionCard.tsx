...
export function ResolutionCard({ round }: { round: RoundVM }) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-6">
      <SectionLabel>Market resolved · Chainlink feed</SectionLabel>

      <div className="grid gap-6 md:grid-cols-[1fr_auto_1fr]">
        <div className="grid grid-cols-2 gap-4 self-center">
          <Stat label="Observed price" value={usd(resolution.observedPriceUsd)} valueClass="text-lg" />
          <Stat label="Liquidation strike" value={usd(resolution.strikePriceUsd)} valueClass="text-lg" />
          <div className="col-span-2">
            <div className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Outcome</div>
            <div className={cn("text-4xl font-bold", outcomeColor)}>{SIDE_LABEL[resolution.outcome]}</div>
          ...
          <div className="col-span-2 flex gap-6 border-t border-border pt-3">
            {resolution.pnl.map((p) => (
              <div key={p.role}>
                ...
                <div className={cn("text-lg font-bold", p.deltaUsdc >= 0 ? "text-yes" : "text-no")}>
                  {p.deltaUsdc >= 0 ? "+" : ""}
                  {p.deltaUsdc} USDC
                </div>
